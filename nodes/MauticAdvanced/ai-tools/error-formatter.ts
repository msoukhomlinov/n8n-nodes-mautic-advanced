...
import { isInvalidGrantError, OAUTH_REFRESH_ERROR_MESSAGE } from '../utils/authErrorHelpers';
...
export function formatApiError(error: unknown, resource: string, operation: string): ErrorEnvelope {
  ...
  if (isInvalidGrantError(error)) {
    return wrapError(
      ...
      'Retry the workflow; if it persists, reconnect the Mautic OAuth2 credential.',
    );
  }
