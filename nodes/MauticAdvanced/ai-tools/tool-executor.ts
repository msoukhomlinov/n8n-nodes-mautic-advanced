...
export async function executeAiTool(
  ...
  try {
    ...
    return JSON.stringify(formatApiError(error, resource, operation));
  }
}
  ...
  switch (operation) {
    ...
    case 'create': {
      ...
      if (typeof createBody.tags === 'string') {
        createBody.tags = (createBody.tags as string)
          .split(',')
          .map((t: string) => t.trim())
          .filter(Boolean);
      }
    ...
    case 'update': {
      ...
      if (typeof updateBody.tags === 'string') {
        updateBody.tags = (updateBody.tags as string)
          .split(',')
          .map((t: string) => t.trim())
          .filter(Boolean);
      }
...
async function executeContactTool(
  ...
  switch (operation) {
    ...
    case 'addToSegments': {
      ...
      const segmentIds =
        typeof params.segmentIds === 'string'
          ? (params.segmentIds as string)
              .split(',')
              .map((s: string) => s.trim())
              .filter(Boolean)
    ...
    case 'addToCampaigns': {
      ...
      const campaignIds =
        typeof params.campaignIds === 'string'
          ? (params.campaignIds as string)
              .split(',')
              .map((s: string) => s.trim())
              .filter(Boolean)
  ...
  switch (operation) {
    ...
    case 'addContact': {
      ...
      const response = await apiRequest(
        context,
        'POST',
        `/segments/${params.id}/contact/${params.contactId}/add`,
      );
      return JSON.stringify(wrapSuccess(resource, operation, response));
    }
    case 'removeContact': {
      ...
      const response = await apiRequest(
        context,
        'POST',
        `/segments/${params.id}/contact/${params.contactId}/remove`,
      );
      return JSON.stringify(wrapSuccess(resource, operation, response));
    }
    default:
  ...
  switch (operation) {
    ...
    case 'create': {
      ...
      if (useV2) {
        ...
        response = await mauticApiRequest.call(
          context,
          'POST',
          '/v2/tags',
          body,
...
async function executeCompanyContactTool(
  ...
  if (!params.companyId)
    return JSON.stringify(
      wrapError(
        ...
        operation,
        ERROR_TYPES.MISSING_REQUIRED_FIELD,
        'A numeric company ID is required.',
        "Call mauticadvanced_company with operation 'getAll' to find the company ID.",
      ),
    );
  if (!params.contactId)
    return JSON.stringify(
      wrapError(
        ...
        ERROR_TYPES.MISSING_REQUIRED_FIELD,
        'A numeric contact ID is required.',
        "Call mauticadvanced_contact with operation 'getAll' to find the contact ID.",
      ),
    );
  ...
  const response = await apiRequest(
    context,
    'POST',
    `/companies/${params.companyId}/contact/${params.contactId}/${action}`,
  );
  return JSON.stringify(wrapSuccess(resource, operation, response));
}
...
async function executeCampaignContactTool(
  ...
  if (!params.campaignId)
    return JSON.stringify(
      wrapError(
        ...
        operation,
        ERROR_TYPES.MISSING_REQUIRED_FIELD,
        'A numeric campaign ID is required.',
        "Call mauticadvanced_campaign with operation 'getAll' to find the campaign ID.",
      ),
    );
  if (!params.contactId)
    return JSON.stringify(
  ...
  const response = await apiRequest(
    context,
    'POST',
    `/campaigns/${params.campaignId}/contact/${params.contactId}/${action}`,
  );
  return JSON.stringify(wrapSuccess(resource, operation, response));
}
...
async function executeContactSegmentTool(
  ...
  if (!params.segmentId)
    return JSON.stringify(
      wrapError(
        ...
        ERROR_TYPES.MISSING_REQUIRED_FIELD,
        'A numeric segment ID is required.',
        "Call mauticadvanced_segment with operation 'getAll' to find the segment ID.",
      ),
    );
  if (!params.contactId)
    return JSON.stringify(
  ...
  const response = await apiRequest(
    context,
    'POST',
    `/segments/${params.segmentId}/contact/${params.contactId}/${action}`,
  );
  return JSON.stringify(wrapSuccess(resource, operation, response));
}
