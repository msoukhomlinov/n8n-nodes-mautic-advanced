...
function getNoteGetAllSchema() {
  return z.object({
    search: z
      ...
      .optional()
      .describe(
        'Partial text match across note content. ALWAYS use this first for any text lookup.',
      ),
    limit: limitSchema,
  });
}
...
function getFieldGetAllSchema() {
  return z.object({
    fieldObject: z
      ...
      .describe(
        "Type of field to list: 'contact' for contact fields, 'company' for company fields.",
      ),
    limit: limitSchema,
  });
}
...
function getContactCreateSchema() {
  return z.object({
    ...
    tags: z.string().optional().describe('Comma-separated list of tags to apply to the contact.'),
    customFields: z
      .string()
      .optional()
      .describe(
...
function getContactUpdateSchema() {
  return z.object({
    ...
    customFields: z.string().optional().describe('JSON object of custom field alias→value pairs.'),
  });
}
...
function getSegmentCreateSchema() {
  return z.object({
    ...
    isGlobal: z
      .boolean()
      .optional()
      .describe('Whether the segment is global (visible to all users).'),
  });
}
...
function getNoteCreateSchema() {
  return z.object({
    ...
    type: z
      ...
      .optional()
      .describe("Note type: 'general' (default), 'email', 'call', or 'meeting'."),
...
function getCategoryCreateSchema() {
  return z.object({
    ...
    bundle: z
      ...
      .describe(
        'Category bundle type (required). Determines which Mautic module this category belongs to.',
      ),
...
function getFieldGetSchema() {
  return z.object({
    id: idSchema,
    fieldObject: z.enum(['contact', 'company']).describe("Type of field: 'contact' or 'company'."),
  });
}
  ...
  switch (typeName) {
    ...
    case 'ZodUnion':
      converted = runtimeZ.union(
        (def.options ?? []).map((o: unknown) => toRuntimeZodSchema(o, runtimeZ)) as [
          any,
          any,
          ...any[],
        ],
