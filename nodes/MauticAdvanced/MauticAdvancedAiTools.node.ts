  ...
  async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
    ...
    const unifiedTool = new RuntimeDynamicStructuredTool({
      ...
      func: async (params: Record<string, unknown>) => {
        ...
        delete operationParams.operation;
        return executeAiTool(this, resource, operation, operationParams, enabledOperations);
      },
    });
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
      ...
      try {
        ...
        const resultJson = await executeAiTool(this, resource, effectiveOp, params, effectiveOps);
        response.push({
